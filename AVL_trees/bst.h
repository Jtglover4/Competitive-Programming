...
protected:
    ...
    virtual void clearit(Node<Key, Value>* root);
    virtual int length(Node<Key, Value>*  root) const;
    virtual bool isBalancedhelper(Node<Key, Value>*  node) const;
...
-----------------------------------------------------
*/
// What's left: Remove, clear, isbalanced, destructor, successor
...
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::clearit(Node<Key, Value>* root)
{
    ...
    clearit(root->getLeft());
    clearit(root->getRight());
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
    ...
    while(true){
        ...
        }else if((temp -> getKey() != key) && (temp -> getRight() == NULL && temp -> getLeft() == NULL)){
...
bool BinarySearchTree<Key, Value>::isBalancedhelper(Node<Key, Value>*  node) const{
    ...
    if(isBalancedhelper(node -> getLeft()) && isBalancedhelper(node -> getRight())){
        ...
        int a = length(node -> getLeft());
        int b = length(node -> getRight());
...
template<typename Key, typename Value>
int BinarySearchTree<Key, Value>::length(Node<Key, Value>*  root) const{
    ...
    if(length(root -> getRight()) > length(root -> getLeft())){
        x = 1 + length(root -> getRight());
        return x;
    }else{
        x = 1 + length(root -> getLeft());
        return x;
    }
